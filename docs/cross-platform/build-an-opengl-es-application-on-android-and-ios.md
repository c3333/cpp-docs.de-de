...
1. Wählen Sie im Dialogfeld **Neues Projekt** unter **Vorlagen**die Option **Visual C++**  > **plattformübergreifend**aus, und wählen Sie dann die Vorlage **opengles-Anwendung (Android, IOS)** aus.
...
- `MyOpenGLESApp.Android.Packaging` erstellt die *APK*-Datei für die Entwicklung auf einem Android-Gerät oder -Emulator. Diese Datei enthält die Ressourcen sowie die Datei „AndroidManifest.xml“, in der Sie Manifesteigenschaften festlegen. Es enthält zudem die Datei *build.xml*, die den Ant-Buildprozess steuert. Es ist standardmäßig als Startprojekt festgelegt, sodass es bereitgestellt und direkt in Visual Studio ausgeführt werden kann.
...
1. Um das automatische Signieren zu aktivieren, aktivieren Sie das Kontrollkästchen **Signierung automatisch verwalten**.
   ...
   ![Automatische Signatur von Xcode](../cross-platform/media/cppmdd-opengles-iosxcodesign.png "Automatisches Signieren von Xcode")
...
Sie können den freigegebenen C++-Code bearbeiten, um allgemeine Funktionen hinzuzufügen oder zu ändern. Ändern Sie die Aufrufe des freigegebenen Codes im `MyOpenGLESApp.Android.NativeActivity` und `MyOpenGLESApp.iOS.Application` Projekte entsprechend. Sie können Präprozessormakros verwenden, um plattformspezifische Abschnitte im gemeinsamen Code anzugeben. Der Präprozessormakro `__ANDROID__` ist bei der Erstellung für Android vordefiniert. Der Präprozessormakro `__APPLE__` ist bei der Erstellung für iOS vordefiniert.
