---
title: Compilerfehler C2827
ms.date: 11/04/2016
f1_keywords:
- C2827
helpviewer_keywords:
- C2827
ms.assetid: cb3e5814-0c92-40e4-b620-98578ae3003a
ms.openlocfilehash: 8da60b895ad5dab0713d736142225053d14301c1
ms.sourcegitcommit: 857fa6b530224fa6c18675138043aba9aa0619fb
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 03/24/2020
ms.locfileid: "80201941"
---
# <a name="compiler-error-c2827"></a>Compilerfehler C2827

"Operator Operator" kann nicht global mit unärer Form überschrieben werden.

Der Operator darf nicht über ein unäres Formular außerhalb eines Objekts verfügen.

### <a name="to-fix-by-using-the-following-possible-solutions"></a>So beheben Sie den Fehler (unterschiedliche Lösungsmöglichkeiten)

1. Legen Sie den überladenen Operator lokal auf ein Objekt.

1. Wählen Sie einen geeigneten unären Operator für die Überlastung aus.
