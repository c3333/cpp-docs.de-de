...
|[CTime:: getDay](#getday)|Gibt den Tag zurück, der durch das `CTime`-Objekt dargestellt wird.|
...
|[CTime:: GetMinute](#getminute)|Gibt die Minute zurück, die durch das `CTime`-Objekt dargestellt wird.|
|[CTime:: getMonth](#getmonth)|Gibt den Monat zurück, der durch das `CTime`-Objekt dargestellt wird.|
|[CTime:: GetSecond](#getsecond)|Gibt die zweite zurück, die durch das `CTime`-Objekt dargestellt wird.|
...
|||
|-|-|
|[Operator +-](#operator_add_-)|Diese Operatoren fügen `CTimeSpan` und `CTime` Objekte hinzu und subtrahieren Sie.|
|[Operator + =,-=](#operator_add_eq_-_eq)|Diese Operatoren fügen ein `CTimeSpan` Objekt zu diesem `CTime` Objekt hinzu und subtrahieren es.|
...
Die Klassen "`CTime`" und "`CTimeSpan`" sind nicht für die Ableitung konzipiert. Da keine virtuellen Funktionen vorhanden sind, beträgt die Größe von `CTime` und `CTimeSpan` Objekten genau 8 Bytes. Die meisten Element Funktionen sind Inline.
...
*time*<br/>
Ein `__time64_t` Zeitwert, der die Anzahl der Sekunden nach dem 1. Januar 1970 UTC ist. Beachten Sie, dass dies an die Ortszeit angepasst wird. Wenn Sie z. b. in New York arbeiten und ein `CTime` Objekt erstellen, indem Sie einen Parameter von 0 übergeben, gibt [ctime:: getMonth](#getmonth) den Wert 12 zurück.
...
Die in einen Datums-und Uhrzeitwert zu konvertierenden MS-DOS-Datums-und Uhrzeitwerte, die in das neue `CTime` Objekt kopiert werden.

*St*<br/>
Eine [SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime) -Struktur, die in einen Datums-/Uhrzeitwert konvertiert und in das neue `CTime`-Objekt kopiert werden soll.

*GRA*<br/>
Eine [FILETIME](/windows/win32/api/minwinbase/ns-minwinbase-filetime) -Struktur, die in einen Datums-/Uhrzeitwert konvertiert und in das neue `CTime`-Objekt kopiert werden soll.
...
- `CTime( const CTime& );` ein `CTime` Objekt aus einem anderen `CTime` Wert erstellt.

- `CTime( __time64_t );` ein `CTime`-Objekt aus einem **__time64_t** Typ erstellt. Dieser Konstruktor erwartet eine UTC-Zeit und konvertiert das Ergebnis in eine lokale Zeit, bevor das Ergebnis gespeichert wird.
...
- erstellt `CTime( WORD, WORD );` ein `CTime`-Objekt aus den angegebenen MS-und Uhrzeitwerten des MS-DOS. Dieser Konstruktor erwartet eine lokale Zeit.

- `CTime( const SYSTEMTIME& );` ein `CTime`-Objekt aus einer `SYSTEMTIME` Struktur erstellt. Dieser Konstruktor erwartet eine lokale Zeit.

- `CTime( const FILETIME& );` ein `CTime`-Objekt aus einer `FILETIME` Struktur erstellt. Wahrscheinlich wird die `CTime FILETIME` Initialisierung nicht direkt verwendet. Wenn Sie ein `CFile` Objekt zum Bearbeiten einer Datei verwenden, ruft `CFile::GetStatus` den Datei Zeitstempel für Sie durch ein `CTime` Objekt ab, das mit einer `FILETIME` Struktur initialisiert wurde. Dieser Konstruktor nimmt eine auf UTC basierende Zeit an und konvertiert den Wert automatisch in lokale Zeit, bevor das Ergebnis gespeichert wird.

   > [!NOTE]
   > Der Konstruktor, der `DBTIMESTAMP`-Parameter verwendet, ist nur verfügbar, wenn "OleDb. h" enthalten ist.
...
Speichert die resultierende Zeit in der referenzierten *DBTS* -Struktur. Bei der `DBTIMESTAMP` Datenstruktur, die von dieser Funktion initialisiert wurde, wird der `fraction` Member auf NULL festgelegt.
...
`GetAsSystemTime` speichert die resultierende Zeit in der referenzierten *timedest* -Struktur. Bei der `SYSTEMTIME` Datenstruktur, die von dieser Funktion initialisiert wurde, wird der `wMilliseconds` Member auf NULL festgelegt.
...
Gibt den Tag zurück, der durch das `CTime`-Objekt dargestellt wird.
...
Diese Funktion ruft `GetLocalTm`auf, der einen internen, statisch zugeordneten Puffer verwendet. Die Daten in diesem Puffer werden aufgrund von Aufrufen anderer `CTime` Member-Funktionen überschrieben.
...
Diese Funktion ruft `GetLocalTm`auf, bei dem ein interner, statisch zugewiesener Puffer verwendet wird. Die Daten in diesem Puffer werden aufgrund von Aufrufen anderer `CTime` Member-Funktionen überschrieben.
...
Ruft eine **struct tm-Struktur** ab, die eine Zerlegung der in diesem `CTime` Objekt enthaltenen Zeit enthält.
...
Gibt die zweite zurück, die durch das `CTime`-Objekt dargestellt wird.
...
Diese Operatoren fügen `CTimeSpan` und `CTime` Objekte hinzu und subtrahieren Sie.
...
`CTime`-Objekte stellen die absolute Zeit dar, `CTimeSpan`-Objekte die relative Zeit darstellen. Die ersten beiden Operatoren ermöglichen das Hinzufügen und Subtrahieren von `CTimeSpan` Objekten zu `CTime` Objekten. Der dritte Operator ermöglicht es Ihnen, ein `CTime` Objekt von einem anderen zu subtrahieren, um ein `CTimeSpan` Objekt zu erhalten.
...
Diese Operatoren fügen ein `CTimeSpan` Objekt zu diesem `CTime` Objekt hinzu und subtrahieren es.
...
Diese Operatoren ermöglichen das Hinzufügen und Subtrahieren eines `CTimeSpan` Objekts zu diesem `CTime` Objekt.
