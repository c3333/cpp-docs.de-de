...
Da C++ das überladen zulässt, können Sie über Ladungen von **sinh** aufrufen, die-oder-Werte verwenden und zurückgeben **`float`** **`long double`** . In einem C-Programm nimmt **sinh** immer an und gibt es zurück **`double`** .
