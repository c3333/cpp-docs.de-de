...
In diesem Dokument werden einige bewährte Methoden zum Optimieren von C++-Programmen in Visual Studio beschrieben.
...
Visual Studio unterstützt *die profilgesteuerte Optimierung* (PGO). Diese Optimierung verwendet Profildaten aus Trainingsausführungen einer instrumentierten Version einer Anwendung, um eine spätere Optimierung der Anwendung voranzutreiben. Das Verwenden der profilgesteuerte Optimierung (PGO) kann zeitaufwendig sein, sodass sie möglicherweise nicht von allen Entwicklern genutzt wird. Wir empfehlen jedoch, die profilgesteuerte Optimierung (PGO) für das endgültige Releasebuild eines Projekts zu verwenden. Weitere Informationen finden Sie unter [Profilgesteuerte Optimierungen](profile-guided-optimizations.md).

Darüber hinaus wurden die *gesamte Programmoptimierung* (auch als Link Time Code Generation bekannt) und die **Optimierungen /O1** und **/O2** verbessert. Allgemein gilt, daß eine Anwendung, die mit einer dieser Optionen kompiliert wurde, sich durch eine höhere Geschwindigkeit auszeichnet als dieselbe Anwendung, die mit einem früheren Compiler kompiliert wurde.

Weitere Informationen finden Sie unter [/GL (Ganze Programmoptimierung)](reference/gl-whole-program-optimization.md) und [/O1, /O2 (Größe minimieren, Geschwindigkeit maximieren)](reference/o1-o2-minimize-size-maximize-speed.md).

### <a name="which-level-of-optimization-to-use"></a>Welche Optimierungsstufe verwendet werden soll
...
Der **Schalter /Gy** ist ebenfalls sehr nützlich. Er generiert ein separates COMDAT für jede Funktion und verleiht dem Linker dadurch mehr Flexibilität beim Entfernen COMDATs, auf die nicht verwiesen wird, und bei der COMDAT-Faltung. Der einzige Nachteil bei der Verwendung von **/Gy** ist, dass es Probleme beim Debuggen verursachen kann. Diese Option sollte also generell verwendet werden. Weitere Informationen finden Sie unter [/Gy (Funktionslevel-Linking aktivieren)](reference/gy-enable-function-level-linking.md).

Für die Verknüpfung in 64-Bit-Umgebungen wird empfohlen, die Option **/OPT:REF,ICF-Linker** zu verwenden, und in 32-Bit-Umgebungen wird **/OPT:REF** empfohlen. Weitere Informationen finden Sie unter [/OPT (Optimierungen)](reference/opt-optimizations.md).

Es wird dringend empfohlen, Debugsymbole zu generieren. Dies gilt sogar für optimierte Releasebuilds. Es wirkt sich nicht auf den generierten Code aus und macht es viel einfacher, Ihre Anwendung zu debuggen, wenn dies erforderlich ist.

### <a name="floating-point-switches"></a>Gleitkommaschalter

Die **/Op-Compileroption** wurde entfernt, und die folgenden vier Compileroptionen, die sich mit Gleitkommaoptimierungen befassen, wurden hinzugefügt:

|||
|-|-|
|**/fp:präzise**|Dies ist die als Standard empfohlene Option, die in den meisten Fällen verwendet werden sollte.|
|**/fp:schnell**|Empfohlen für die Fälle, in denen die Leistung von höchster Wichtigkeit ist, z. B. bei Spielen. Dies führt zur schnellsten Programmausführung.|
|**/fp:streng**|Empfohlen für die Fälle, in denen präzise Gleitkommaausnahmen und IEEE-Verhalten gewünscht werden. Dies führt zur langsamsten Programmausführung.|
|**/fp:außer[-]**|Kann in Verbindung mit **/fp:strict** oder **/fp:precise**verwendet werden, aber nicht **/fp:fast**.|
...
## <a name="optimization-declspecs"></a>Optimierungsdeclspecs
...
## <a name="optimization-pragmas"></a>Optimierungspragmas

Code kann auch mithilfe einer Reihe von nützlichen Pragmas optimiert werden. Die erste, die wir `#pragma optimize`besprechen werden, ist:
...
Außerdem steht mit `#pragma inline_depth` ein nützliches Pragma zur Beschränkung der Tiefe beim Inlining zur Verfügung. Dies ist in der Regel in Situationen nützlich, in denen Sie versuchen, die Größe eines Programms oder einer Funktion zu begrenzen. Weitere Informationen finden Sie unter [inline_depth](../preprocessor/inline-depth.md).

## <a name="__restrict-and-__assume"></a>__restrict \_und _assume

Es gibt ein paar Schlüsselwörter in Visual Studio, die die Leistung unterstützen können: [__restrict](../cpp/extension-restrict.md) und [__assume](../intrinsics/assume.md).
...
Ein Zeiger, der `__restrict` mit geändert wird, wird als *__restrict Zeiger*bezeichnet. Ein __restrict Zeiger ist ein Zeiger, auf \_den nur über den _restrict Zeiger zugegriffen werden kann. Mit anderen Worten, ein anderer Zeiger kann nicht verwendet \_werden, um auf die Daten zuzugreifen, auf die der _restrict Zeiger zeigt.

`__restrict`kann ein leistungsstarkes Tool für den Microsoft C++-Optimierer sein, aber verwenden Sie es mit großer Sorgfalt. Wenn Sie dieses Tool unangemessen verwenden, führt der Optimierer u. U. eine Optimierung durch, die zum Abbruch der Anwendung führt.

Das `__restrict` Schlüsselwort ersetzt den **/Oa-Schalter** aus früheren Versionen.
...
Für `__assume` gelten einige Beschränkungen. Es handelt sich hierbei genau wie bei `__restrict` nur um einen Vorschlag, der vom Compiler ggf. ignoriert werden kann. Zum anderen funktioniert `__assume` derzeit nur mit Ungleichungen aus Variablen und Konstanten. Es propagiert keine symbolischen Ungleichheiten, z. B. unter der Annahme (ein < b).

## <a name="intrinsic-support"></a>Intrinsische Unterstützung

Systeminterne Funktionen werden bei Funktionsaufrufen verwendet, bei denen der Compiler auf ein systeminternes Wissen über den Aufruf zurückgreifen kann und keine Funktion in einer Bibliothek aufzurufen braucht. Stattdessen wird der Code für diese Funktion vom Compiler ausgegeben. Die Headerdatei \<intrin.h enthält> alle verfügbaren Systemeigendaten für jede der unterstützten Hardwareplattformen.
...
Weitere Informationen finden Sie unter [Compiler Intrinsics](../intrinsics/compiler-intrinsics.md).
