...
**/Gr** gibt die `__fastcall` Aufruf Konvention für alle Funktionen außer C++-Member-Funktionen, `main`Funktionen mit dem Namen und Funktionen `__cdecl`an `__stdcall`, die `__vectorcall`als, oder markiert sind. Alle `__fastcall`-Funktionen müssen Prototypen haben. Diese Aufrufkonvention ist nur in Compilern verfügbar, die auf x86 abzielen, und wird von Compilern ignoriert, die auf andere Architekturen abzielen.

**/Gz** gibt die `__stdcall` Aufruf Konvention für alle Funktionen außer C++-Member-Funktionen, `main`Funktionen mit dem Namen und Funktionen `__cdecl`an `__fastcall`, die `__vectorcall`als, oder markiert sind. Alle `__stdcall`-Funktionen müssen Prototypen haben. Diese Aufrufkonvention ist nur in Compilern verfügbar, die auf x86 abzielen, und wird von Compilern ignoriert, die auf andere Architekturen abzielen.

**/GV** gibt die `__vectorcall` Aufruf Konvention für alle Funktionen außer C++-Element Funktionen, Funktionen `main`mit dem Namen, `vararg` Funktionen mit einer Variablen Argumentliste oder Funktionen an, die mit `__cdecl`einem `__stdcall`in Konflikt `__fastcall` stehenden-,-oder-Attribut gekennzeichnet sind. Diese Aufrufkonvention ist nur auf x86- und x64-Architekturen verfügbar, die „/arch:SSE2“ und höher unterstützen, und wird von Compilern ignoriert, die auf die ARM-Architektur abzielen.
