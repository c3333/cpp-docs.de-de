...
1. Verwenden **`new`** Sie, um Datenstrukturen auf dem Heap zuzuordnen und **`delete`** deren Zuteilung aufzuteilen, wie in den folgenden Beispielen gezeigt:
...
1. Verwenden Sie den- **`new`** Operator, der einen Zeiger auf das-Objekt zurückgibt, um Objekte auf dem Heap zuzuordnen. Verwenden Sie den- **`delete`** Operator, um Sie zu löschen.
   ...
   Wenn das Argument für den `CPerson` Konstruktor ein Zeiger auf ist **`char`** , lautet die Anweisung für die Frame Zuordnung wie folgt:
