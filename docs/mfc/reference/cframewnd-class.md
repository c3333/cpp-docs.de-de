...
|[CFrameWnd:: ansichtsansicht](#createview)|Erstellt eine Sicht innerhalb eines Frames, die nicht von `CView`abgeleitet ist.|
...
|[CFrameWnd:: endmodalstate](#endmodalstate)|Beendet den modalen Zustand des Rahmen Fensters. Aktiviert alle Fenster, die durch `BeginModalState`deaktiviert werden.|
...
|[CFrameWnd:: initialupdateframe](#initialupdateframe)|Bewirkt, dass die `OnInitialUpdate` Member-Funktion, die zu allen Ansichten im Rahmen Fenster gehört, aufgerufen wird.|
...
|[CFrameWnd:: rectdefault](#rectdefault)|Übergeben Sie diese statische `CRect` als Parameter, wenn Sie ein `CFrameWnd` Objekt erstellen, damit Windows die anfängliche Größe und Position des Fensters auswählen kann.|

## <a name="remarks"></a>Hinweise

Um ein nützliches Rahmen Fenster für die Anwendung zu erstellen, leiten Sie eine Klasse von `CFrameWnd`ab. Fügen Sie der abgeleiteten Klasse Element Variablen hinzu, um Daten zu speichern, die für Ihre Anwendung spezifisch sind. Implementieren Sie Meldungshandler-Memberfunktionen und eine Meldungszuordnung in der abgeleiteten Klasse, um anzugeben, was passiert, wenn Meldungen an das Fenster weitergeleitet werden.
...
Bevor Sie entweder `Create` oder `LoadFrame`aufgerufen haben, müssen Sie das Rahmen Fenster Objekt auf dem Heap mithilfe des C++ Operators **New** erstellen. Vor dem Aufrufen von `Create`können Sie auch eine Fenster Klasse mit der globalen Funktion [AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) registrieren, um das Symbol und die Klassen Stile für den Frame festzulegen.

Verwenden Sie die `Create` Member-Funktion, um die Erstellungs Parameter des Frames als unmittelbare Argumente zu übergeben.

`LoadFrame` erfordert weniger Argumente als `Create`und ruft stattdessen die meisten Standardwerte aus den Ressourcen ab, einschließlich der Beschriftung, des Symbols, der Zugriffstasten Tabelle und des Menüs des Frames. Alle diese Ressourcen müssen über dieselbe Ressourcen-ID verfügen (z. b. IDR_MAINFRAME), damit Sie über `LoadFrame`zugänglich ist.

Wenn ein `CFrameWnd` Objekt Sichten und Dokumente enthält, werden diese indirekt durch das Framework anstatt direkt vom Programmierer erstellt. Das `CDocTemplate`-Objekt orchestriert die Erstellung des Frames, die Erstellung der enthaltenden Sichten und die Verbindung der Ansichten mit dem entsprechenden Dokument. Mit den Parametern des `CDocTemplate` Konstruktors werden die `CRuntimeClass` der drei beteiligten Klassen angegeben (Dokument, Frame und Ansicht). Ein `CRuntimeClass`-Objekt wird vom Framework verwendet, um neue Frames dynamisch zu erstellen, wenn Sie vom Benutzer angegeben werden (z. b. mit dem Befehl File New oder dem Fenster Multiple Document Interface (MDI) Window New).

Eine von `CFrameWnd` abgeleitete Frame Fenster Klasse muss mit DECLARE_DYNCREATE deklariert werden, damit der obige RUNTIME_CLASS Mechanismus ordnungsgemäß funktioniert.

Eine `CFrameWnd` enthält Standard Implementierungen zum Ausführen der folgenden Funktionen eines Hauptfensters in einer typischen Anwendung für Windows:

- Ein `CFrameWnd` Rahmen Fenster verfolgt eine derzeit aktive Ansicht, die vom aktiven Windows-Fenster oder dem aktuellen Eingabefokus unabhängig ist. Wenn der Frame reaktiviert wird, wird die aktive Ansicht durch Aufrufen von `CView::OnActivateView`benachrichtigt.

- Befehlsnachrichten und viele allgemeine Frame Benachrichtigungs Meldungen, einschließlich derjenigen, die von den Funktionen `OnSetFocus`, `OnHScroll`und `OnVScroll` von `CWnd`verarbeitet werden, werden von einem `CFrameWnd` Rahmen Fenster an die derzeit aktive Ansicht delegiert.
...
- Ein `CFrameWnd` Rahmen Fenster verwaltet die Positionierung der Steuer leisten, Ansichten und anderer untergeordneter Fenster im Client Bereich des Rahmen Fensters. Ein Rahmen Fenster führt außerdem eine Leerlaufzeit Aktualisierung der Symbolleiste und anderer Steuer leisten Schaltflächen aus. Ein `CFrameWnd` Rahmen Fenster verfügt auch über Standard Implementierungen von Befehlen zum Umschalten und Deaktivieren der Symbolleiste und der Statusleiste.
...
- Ein `CFrameWnd` Rahmen Fenster verfügt über eine optionale Zugriffstasten Tabelle, die Tastaturbeschleuniger automatisch übersetzt.

- Für ein `CFrameWnd` Rahmen Fenster ist eine optionale Hilfe-ID mit `LoadFrame` festgelegt, die für die kontextbezogene Hilfe verwendet wird. Ein Rahmen Fenster ist der hauptorchestrator von semimodalen Zuständen, z. b. kontextabhängige Hilfe (UMSCHALT + F1) und Druck-Vorschau-Modi.
...
- Wenn das Rahmen Fenster das Hauptanwendungsfenster (d. h. `CWinThread::m_pMainWnd`) ist, fordert das Rahmen Fenster den Benutzer auf, geänderte Dokumente (für `OnClose` und `OnQueryEndSession`) zu speichern, wenn der Benutzer die Anwendung schließt.
...
*lpszClassName*<br/>
Verweist auf eine mit NULL endenden Zeichenfolge, die die Windows-Klasse benennt. Der Klassenname kann ein beliebiger Name sein, der mit der `AfxRegisterWndClass` globalen Funktion oder der `RegisterClass` Windows-Funktion registriert ist. Wenn der Wert NULL ist, werden die vordefinierten Standard `CFrameWnd` Attribute verwendet.
...
Verwenden Sie diese Member-Funktion, um Ansichten zu erstellen, die nicht `CView`innerhalb eines Frames abgeleitet werden. Nachdem Sie `CreateView`aufgerufen haben, müssen Sie die Ansicht manuell auf "aktiv" festlegen und so festlegen, dass Sie sichtbar ist. Diese Aufgaben werden nicht automatisch von `CreateView`ausgeführt.
...
Rufen Sie diese Member-Funktion auf, um einen Zeiger auf den aktuellen `CDocument` zu erhalten, der der aktuellen aktiven Ansicht angefügt ist.
...
Rufen Sie diese Member-Funktion auf, um einen Zeiger auf die aktive Ansicht (sofern vorhanden) zu erhalten, die an ein Rahmen Fenster (`CFrameWnd`) angefügt ist.
...
Diese Funktion gibt NULL zurück, wenn Sie für ein MDI-Hauptrahmen Fenster (`CMDIFrameWnd`) aufgerufen wird. In einer MDI-Anwendung ist dem MDI-Hauptrahmen Fenster keine Ansicht zugeordnet. Stattdessen verfügt jedes einzelne untergeordnete Fenster (`CMDIChildWnd`) über mindestens eine zugeordnete Sicht. Die aktive Ansicht in einer MDI-Anwendung kann abgerufen werden, indem Sie zuerst das aktive untergeordnete MDI-Fenster Suchen und dann die aktive Ansicht für dieses untergeordnete Fenster suchen. Das aktive untergeordnete MDI-Fenster kann durch Aufrufen der Funktion `MDIGetActive` oder `GetActiveFrame` gefunden werden, wie im folgenden gezeigt:

[!code-cpp[NVC_MFCWindowing#2](../../mfc/reference/codesnippet/cpp/cframewnd-class_2.cpp)]

##  <a name="getcontrolbar"></a>CFrameWnd:: getcontrolbar

`GetControlBar` aufrufen, um Zugriff auf die Steuerleiste zu erhalten, die mit der ID verknüpft ist.
...
`GetControlBar` gibt die Steuerleiste zurück, auch wenn Sie unverankert ist und daher aktuell kein untergeordnetes Fenster des Frames ist.
...
Anschließend können Sie den Inhalt von `CDockState` mithilfe `CDockState::SaveState` oder `Serialize`in den Speicher schreiben. Wenn Sie die Steuer leisten später in einem vorherigen Zustand wiederherstellen möchten, laden Sie den Zustand mit `CDockState::LoadState` oder `Serialize`, und wenden Sie dann `SetDockState` an, um den vorherigen Zustand auf die Steuer leisten des Rahmen Fensters anzuwenden.
...
Ruft `IntitialUpdateFrame` nach dem Erstellen eines neuen Frames mit `Create`auf.
...
Die Einstellungen, die Sie wiederherstellen möchten, müssen in die Registrierung geschrieben werden, bevor Sie `LoadBarState`aufgerufen werden. Schreiben Sie die Informationen in die Registrierung, indem Sie [CWinApp:: settregistrykey](../../mfc/reference/cwinapp-class.md#setregistrykey)aufrufen. Schreiben Sie die Informationen in die INI-Datei, indem Sie [SaveBarState](#savebarstate)aufrufen.
...
Erstellen Sie ein `CFrameWnd`-Objekt in zwei Schritten. Rufen Sie zuerst den-Konstruktor auf, der das `CFrameWnd`-Objekt erstellt, und rufen Sie dann `LoadFrame`auf, das das Windows-Rahmen Fenster und zugehörige Ressourcen lädt und das Rahmen Fenster an das `CFrameWnd`-Objekt anfügt. Der Parameter " *nidresource* " gibt das Menü, die Zugriffstasten Tabelle, das Symbol und die Zeichen folgen Ressource des Titels für das Rahmen Fenster an.

Verwenden Sie die `Create` Member-Funktion statt `LoadFrame`, wenn Sie alle Erstellungs Parameter des Frame Fensters angeben möchten.

Das Framework ruft `LoadFrame` auf, wenn ein Rahmen Fenster mithilfe eines Dokumentvorlagen Objekts erstellt wird.

Das Framework verwendet das *pContext* -Argument, um die Objekte anzugeben, die mit dem Rahmen Fenster verbunden werden sollen, einschließlich aller enthaltenen Ansichts Objekte. Sie können das *pContext* -Argument auf NULL festlegen, wenn Sie `LoadFrame`aufzurufen.
...
-Anweisung für die Meldungs Zuordnung der `CFrameWnd` Klasse und außerdem einen Zugriffstasten-Tabelleneintrag hinzufügen, normalerweise UMSCHALT + F1, um diese Element Funktion zu aktivieren.

Wenn es sich bei der Anwendung um einen OLE-Container handelt, werden `OnContextHelp` alle in dem Rahmen Fenster Objekt enthaltenen direkten Elemente in den Hilfe Modus versetzt. Der Cursor wechselt zu einem Pfeil und einem Fragezeichen, und der Benutzer kann dann den Mauszeiger bewegen und die linke Maustaste drücken, um ein Dialogfeld, ein Fenster, ein Menü oder eine Befehls Schaltfläche auszuwählen. Diese Member-Funktion Ruft die Windows-Funktion `WinHelp` mit dem Hilfe Kontext des-Objekts unter dem Cursor auf.
...
Die Standard Implementierung dieser Funktion erstellt, wenn möglich, ein `CView` Objekt aus den Informationen, die in *pContext*bereitgestellt werden.

Überschreiben Sie diese Funktion, um Werte zu überschreiben, die im `CCreateContext` Objekt übergebenen werden, oder um die Art und Weise zu ändern, wie Steuerelemente im Haupt Client Bereich des Frame Fensters Die `CCreateContext` Elemente, die Sie überschreiben können, werden in der [ckreatecontext](../../mfc/reference/ccreatecontext-structure.md) -Klasse beschrieben.

> [!NOTE]
>  Ersetzen Sie keine Werte, die in der `CREATESTRUCT` Struktur übermittelt werden. Sie dienen nur zur Informations Verwendung. Wenn Sie das anfängliche Fenster Rechteck außer Kraft setzen möchten, überschreiben Sie z. b. die `CWnd` Member-Funktion [prekreatewindow](../../mfc/reference/cwnd-class.md#precreatewindow).
...
Die Standard Implementierung dieser Member-Funktion Ruft die `CWnd` Member-`RepositionBars` Funktion auf, um alle Steuer leisten im Frame und im Haupt Client Fenster (normalerweise ein `CView` oder MdiClient) neu zu positionieren.
...
*bbenachrichtigen*<br/>
Gibt an, ob die Ansicht über die Aktivierung benachrichtigt werden soll. TRUE gibt an, dass `OnActivateView` für die neue Ansicht aufgerufen wird. FALSE gibt an, dass dies nicht der Fall ist.

### <a name="remarks"></a>Hinweise

Das Framework ruft diese Funktion automatisch auf, wenn der Benutzer den Fokus auf eine Ansicht innerhalb des Rahmen Fensters ändert. Sie können `SetActiveView` explizit aufzurufen, um den Fokus auf die angegebene Ansicht zu ändern.

##  <a name="setdockstate"></a>CFrameWnd:: setdockstate

Mit dieser Member-Funktion können Sie die in einem `CDockState` Objekt gespeicherten Zustandsinformationen auf die Steuer leisten des Rahmen Fensters anwenden.
...
Wenn Sie einen früheren Zustand der Steuer leisten wiederherstellen möchten, können Sie den gespeicherten Zustand mit `CDockState::LoadState` oder `Serialize`laden und dann mithilfe von `SetDockState` auf die Steuer leisten des Rahmen Fensters anwenden. Der vorherige Zustand wird im `CDockState` Objekt mit gespeichert `GetDockState`
...
*nprogresspos*<br/>
Gibt die festzulegende Position an. Sie muss innerhalb des Bereichs liegen, der durch `SetProgressBarRange`festgelegt wird.
