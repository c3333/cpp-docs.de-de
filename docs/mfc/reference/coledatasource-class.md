...
|[COleDataSource::D elaysetdata](#delaysetdata)|Wird für jedes Format aufgerufen, das in unterstützt wird `OnSetData` .|
...
Nachdem der Aufruf an `CacheData` den `ptd` -Member von `lpFormatEtc` und den Inhalt von *lpstgmedium* im Besitz des Datenobjekts, nicht des Aufrufers, ist.
...
Verwenden Sie diese Funktion, wenn Sie die Daten nicht durch ein-Objekt bereitstellen `CFile` . Wenn Sie die Daten über ein-Objekt bereitstellen `CFile` , können Sie die-Member-Funktion von " [Delta Data](#delayrenderfiledata) " aufrufen. Weitere Informationen zum verzögerten Rendering, wie von MFC verarbeitet, finden Sie im Artikel [Datenobjekte und Datenquellen: Manipulation](../../mfc/data-objects-and-data-sources-manipulation.md).
...
Verwenden Sie diese Funktion, wenn Sie ein-Objekt verwenden, `CFile` um die Daten bereitzustellen. Wenn Sie kein-Objekt verwenden, müssen Sie die-Member-Funktion von " `CFile` [Delta Data](#delayrenderdata) " aufrufen. Weitere Informationen zum verzögerten Rendering, wie von MFC verarbeitet, finden Sie im Artikel [Datenobjekte und Datenquellen: Manipulation](../../mfc/data-objects-and-data-sources-manipulation.md).
...
[Onsetdata](#onsetdata) wird von Framework aufgerufen, wenn dies geschieht. Diese Option wird nur verwendet, wenn das Framework die Datenquelle aus [COleServerItem:: GetDataSource](../../mfc/reference/coleserveritem-class.md#getdatasource)zurückgibt. Wenn `DelaySetData` nicht aufgerufen wird, `OnSetData` wird die Funktion nie aufgerufen. `DelaySetData`sollte für jede Zwischenablage oder jedes Format aufgerufen werden, das `FORMATETC` Sie unterstützen.
...
Wenn *lpstgmedium* ->  *TYMED* TYMED_NULL ist, sollte der entsprechend der Angabe `STGMEDIUM` durch *lpformatetc->TYMED*zugeordnet und ausgefüllt werden. Wenn dies TYMED_NULL nicht der Fall ist, `STGMEDIUM` sollte die mit den Daten aufgefüllt werden.
...
Fügt die Daten, die im-Objekt enthalten sind, `COleDataSource` nach dem Aufrufen einer der folgenden Funktionen in die Zwischenablage ein: [CacheData](#cachedata), [CacheGlobalData](#cacheglobaldata), [Delta-renderData](#delayrenderdata)oder [Delta Data](#delayrenderfiledata).
