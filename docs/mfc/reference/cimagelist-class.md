...
|[CImageList::D eletetempmap](#deletetempmap)|Wird vom [CWinApp](../../mfc/reference/cwinapp-class.md) -Leerlaufzeit Handler aufgerufen, um ein temporäres Objekt zu löschen, das `CImageList` von erstellt wurde `FromHandle` .|
...
|[CImageList:: FromHandle](#fromhandle)|Gibt einen Zeiger auf ein-Objekt zurück, `CImageList` Wenn ein Handle für eine Bildliste angegeben ist. Wenn ein `CImageList`-Objekt nicht an das Handle angefügt ist, wird ein temporäres `CImageList`-Objekt erstellt und angefügt.|
|[CImageList:: fromlenker permanent](#fromhandlepermanent)|Gibt einen Zeiger auf ein-Objekt zurück, `CImageList` Wenn ein Handle für eine Bildliste angegeben ist. Wenn ein- `CImageList` Objekt nicht an das Handle angefügt ist, wird NULL zurückgegeben.|
...
|[CImageList:: geout afehandle](#getsafehandle)|Ruft ab `m_hImageList` .|
...
|Name|BESCHREIBUNG|
|----------|-----------------|
|[CImageList:: Operator HIMAGELIST](#operator_himagelist)|Gibt die an angefügte HIMAGELIST zurück `CImageList` .|
...
Weitere Informationen zum Verwenden von finden Sie unter Steuer `CImageList` [Elemente](../../mfc/controls-mfc.md) und [Verwenden von CImageList](../../mfc/using-cimagelist.md).
...
Mit dieser Funktion können Sie eine Bildliste an ein-Objekt anfügen `CImageList` .
...
Diese Funktion erstellt eine temporäre Bildliste, die für das Ziehen verwendet wird. Das Bild kombiniert das angegebene Bild und seine Maske mit dem aktuellen Cursor. Als Reaktion auf nachfolgende WM_MOUSEMOVE Meldungen können Sie das Zieh Bild mithilfe der Member- `DragMove` Funktion verschieben. Um den Zieh Vorgang zu beenden, können Sie die- `EndDrag` Member-Funktion verwenden.
...
*nFlags*<br/>
Gibt den Typ der zu erstellenden Bildliste an. Dieser Parameter kann eine Kombination der folgenden Werte sein, kann jedoch nur einen der-Werte einschließen `ILC_COLOR` .
...
Sie erstellen einen `CImageList` in zwei Schritten. Zuerst wird der-Konstruktor aufgerufen und dann aufgerufen `Create` , wodurch die Bildliste erstellt und an das-Objekt angefügt wird `CImageList` .
...
Wird automatisch vom `CWinApp` Leerlaufzeit Handler aufgerufen, `DeleteTempMap` Löscht alle temporären `CImageList` , von [FromHandle](#fromhandle)erstellten temporären Objekte, zerstört jedoch keine Handles ( `hImageList` ), die den Objekten vorübergehend zugeordnet sind `ImageList` .
...
*PT*<br/>
Eine [Punkt](/windows/win32/api/windef/ns-windef-point) Struktur, die die x-und y-Koordinaten enthält, in denen das Bild gezeichnet wird.

*RT*<br/>
Eine [Größen](/windows/win32/api/windef/ns-windef-size) Struktur, die die Größe des Bilds angibt, das gezeichnet werden soll.

*ptorigin*<br/>
Eine [Punkt](/windows/win32/api/windef/ns-windef-point) Struktur, die die x-und y-Koordinaten enthält, die die linke obere Ecke des Zeichnungs Vorgangs in Bezug auf das Bild selbst angeben. Pixel des Bilds, die sich Links von der x-Koordinate und oberhalb der y-Koordinate befinden, werden nicht gezeichnet.
...
Gibt einen Zeiger auf ein-Objekt zurück, `CImageList` Wenn ein Handle für eine Bildliste angegeben ist.
...
## <a name="cimagelistfromhandlepermanent"></a><a name="fromhandlepermanent"></a>CImageList:: fromlenker permanent
...
*lpPoint*<br/>
Adresse einer [Punkt](/windows/win32/api/windef/ns-windef-point) Struktur, die die aktuelle Zieh Position empfängt.
...
## <a name="cimagelistgetimageinfo"></a><a name="getimageinfo"></a>CImageList:: getimagan FO
...
## <a name="cimagelistgetsafehandle"></a><a name="getsafehandle"></a>CImageList:: geout afehandle
...
Verwenden Sie diesen Operator, um das angefügte Handle des-Objekts zu erhalten `CImageList` .
...
Bei Erfolg ein Handle für die Bildliste, die durch das-Objekt dargestellt wird, `CImageList` andernfalls NULL.
...
Die zurückgegebene Version **`int`** gibt den NULL basierten Index des Bilds zurück, wenn erfolgreich, andernfalls-1.
...
## <a name="cimagelistsetdragcursorimage"></a><a name="setdragcursorimage"></a>CImageList:: setdragcurrsorimage
...
Da die Zieh Funktionen das neue Bild während eines Zieh Vorgangs verwenden, sollten Sie die Windows [ShowCursor](/windows/win32/api/winuser/nf-winuser-showcursor) -Funktion verwenden, um den eigentlichen Mauszeiger nach dem Aufruf von auszublenden `CImageList::SetDragCursorImage` . Andernfalls weist das System möglicherweise zwei Mauszeiger auf die Dauer des Zieh Vorgangs auf.

## <a name="cimagelistsetimagecount"></a><a name="setimagecount"></a>CImageList:: Abbild magecount

Mit dieser Member-Funktion können Sie die Anzahl der Bilder in einem-Objekt zurücksetzen `CImageList` .
...
## <a name="cimagelistsetoverlayimage"></a><a name="setoverlayimage"></a>CImageList:: Server-layimage
