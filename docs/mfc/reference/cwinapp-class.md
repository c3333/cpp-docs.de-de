...
|[CWinApp:: Unregister](#unregister)|Hebt die Registrierung aller bekannten Elemente auf, die vom-Objekt registriert werden `CWinApp` .|
...
Jede Anwendung, die die Microsoft Foundation-Klassen verwendet, kann nur ein von abgeleitetes Objekt enthalten `CWinApp` . Dieses Objekt wird erstellt, wenn andere globale C++-Objekte erstellt werden und bereits verfügbar sind, wenn Windows die- `WinMain` Funktion aufruft, die von der Microsoft Foundation Class-Bibliothek bereitgestellt wird. Deklarieren Sie `CWinApp` das abgeleitete Objekt auf globaler Ebene.

Wenn Sie eine Anwendungsklasse von ableiten `CWinApp` , überschreiben Sie die [InitInstance](#initinstance) -Member-Funktion, um das Hauptfenster Objekt Ihrer Anwendung zu erstellen.
...
- [Afxgetappname](application-information-and-management.md#afxgetappname) Ruft einen Zeiger auf eine Zeichenfolge ab, die den Namen der Anwendung enthält. Wenn Sie einen Zeiger auf das-Objekt haben `CWinApp` , verwenden Sie alternativ, `m_pszExeName` um den Namen der Anwendung zu erhalten.

Weitere Informationen zur-Klasse finden Sie unter [CWinApp: die Anwendungsklasse](../../mfc/cwinapp-the-application-class.md) `CWinApp` , einschließlich einer Übersicht über Folgendes:
...
Rufen Sie [hideapplication](#hideapplication) auf, bevor Sie aufrufen `CloseAllDocuments` .
...
*lpszappname*<br/>
Eine auf NULL endenden Zeichenfolge, die den von Windows verwendeten Anwendungsnamen enthält. Wenn dieses Argument nicht angegeben wird oder den Wert NULL hat, `CWinApp` verwendet die Ressourcen Zeichenfolge AFX_IDS_APP_TITLE oder den Dateinamen der ausführbaren Datei.

### <a name="remarks"></a>Bemerkungen

Sie sollten ein globales Objekt der von `CWinApp` abgeleiteten Klasse erstellen. Sie können nur ein `CWinApp` Objekt in der Anwendung haben. Der-Konstruktor speichert einen Zeiger auf das `CWinApp` -Objekt, sodass `WinMain` die Element Funktionen des-Objekts abrufen kann, um die Anwendung zu initialisieren und auszuführen.
...
Gibt die gleichen Werte wie zurück `AfxMessageBox` .

### <a name="remarks"></a>Bemerkungen

Verwenden Sie diese Member-Funktion nicht, um ein Meldungs Feld zu öffnen. Verwenden Sie `AfxMessageBox` stattdessen.
...
Um eine einfachere und optimierte Methode zum Implementieren eines warte Cursors zu implementieren, verwenden Sie `CWaitCursor` .
...
Verwenden Sie diese Member-Funktion aus dem Konstruktor der von `CWinApp` abgeleiteten Klasse, um HTMLHelp für die Hilfe Ihrer Anwendung zu verwenden.
...
Diese Funktion wird in der Regel von der `InitInstance` außer Kraft Setzung aufgerufen, damit die Benutzer Ihrer Anwendung Datendateien öffnen können, wenn Sie im Windows-Datei-Manager auf die Dateien doppelklicken.
...
Wird von Framework innerhalb der Member- `Run` Funktion aufgerufen, um diese Instanz der Anwendung zu beenden.
...
Der Exitcode der Anwendung; der Wert 0 gibt keine Fehler an, und Werte größer als 0 deuten auf einen Fehler hin. Dieser Wert wird als Rückgabewert von verwendet `WinMain` .
...
Die Zeitspanne, die das Framework darauf wartet, dass die Wiederherstellungs Rückruffunktion zurückgegeben wird, ist das Ping-Intervall. Sie können das Ping-Intervall anpassen, indem Sie `CWinApp::GetApplicationRecoveryPingInterval` oder einen benutzerdefinierten Wert für angeben `RegisterWithRestartManager` .
...
*POS*<br/>
Ein Verweis auf einen Positionswert, der von einem vorherigen-Befehl an `GetNextDocTemplate` oder [GetFirstDocTemplatePosition](#getfirstdoctemplateposition)zurückgegeben wurde. Der Wert wird durch diesen-Befehl auf die nächste Position aktualisiert.
...
Sie können `GetNextDocTemplate` in einer Forward-Iterations Schleife verwenden, wenn Sie die ursprüngliche Position mit einem-Aufrufpunkt einrichten `GetFirstDocTemplatePosition` .
...
Diese Memberfunktion unterstützt Hexadezimalnotation für den Wert in der INI-Datei. Wenn Sie eine Ganzzahl mit Vorzeichen abrufen, sollten Sie den Wert in einen umwandeln **`int`** .
...
*dwdata*<br/>
Gibt zusätzliche Daten an. Der verwendete Wert hängt vom Wert des *ncmd* -Parameters ab. Der Standardwert ist, `0x000F` was [HH_HELP_CONTEXT](/previous-versions/windows/desktop/htmlhelp/hh-help-context-command)bedeutet.
...
*nidcursor*<br/>
Ein **OCR_** Manifest-Konstantenbezeichner, der einen vordefinierten Windows-Cursor angibt. Sie müssen über verfügen, damit Sie auf `#define OEMRESOURCE` `#include \<afxwin.h>` die **OCR_** Konstanten in Windows zugreifen können. Micha.
...
Verwenden Sie die- `LoadOEMCursor` oder [loadstandardcursor](#loadstandardcursor) -Member-Funktion, um auf die vordefinierten Windows-Cursor zuzugreifen.
...
*nididaktion*<br/>
Ein **OIC_** Manifest-Konstante Bezeichner, der ein vordefiniertes Windows-Symbol angibt. Sie müssen über verfügen, damit Sie `#define OEMRESOURCE` `#include \<afxwin.h>` auf die **OIC_** Konstanten in Windows zugreifen können. Micha.
...
Verwenden Sie die- `LoadOEMIcon` oder die [loadstandarmemon](#loadstandardicon) -Element Funktion, um auf die vordefinierten Windows-Symbole zuzugreifen.
...
Verwenden Sie die- `LoadStandardCursor` oder [loadoemcursor](#loadoemcursor) -Member-Funktion, um auf die vordefinierten Windows-Cursor zuzugreifen.
...
Der Typ dieses Datenmembers ist der Enumerationstyp AFX_HELP_TYPE, der in der-Klasse definiert ist `CWinApp` .
...
- Um die Hilfe der Anwendung auf die HTML-Hilfe festzulegen, nennen Sie [sethelpmode](#sethelpmode) , und geben Sie an `afxHTMLHelp` .

- Um die Hilfe der Anwendung auf WinHelp festzulegen, geben Sie an, `SetHelpMode` und geben Sie an `afxWinHelp` .

## <a name="cwinappm_hinstance"></a><a name="m_hinstance"></a> CWinApp:: m_hInstance

Entspricht dem *HINSTANCE* -Parameter, der von Windows an übergeben wird `WinMain` .
...
Entspricht dem *lpCmdLine* -Parameter, der von Windows an übergeben wird `WinMain` .
...
Entspricht dem *nCmdShow* -Parameter, der von Windows an übergeben wird `WinMain` .
...
Sie sollten als Argument übergeben werden, `m_nCmdShow` Wenn Sie [CWnd:: ShowWindow](../../mfc/reference/cwnd-class.md#showwindow) für das Hauptfenster der Anwendung aufzurufen. `m_nCmdShow` ist eine öffentliche Variable vom Typ **`int`** .
...
> [!NOTE]
> Wenn Sie einen Wert zuweisen `m_pszAppName` , muss er dynamisch auf dem Heap zugeordnet werden. Der `CWinApp` Dekonstruktor Ruft mit diesem Zeiger **Free**() auf. Sie möchten die `_tcsdup` ()-Lauf Zeit Bibliotheksfunktion verwenden, um die Zuordnung durchzuführen. Außerdem müssen Sie den dem aktuellen Zeiger zugeordneten Arbeitsspeicher freigeben, bevor Sie einen neuen Wert zuweisen. Zum Beispiel:
...
> [!NOTE]
> Wenn Sie einen Wert zuweisen `m_pszExeName` , muss er dynamisch auf dem Heap zugeordnet werden. Der `CWinApp` Dekonstruktor Ruft mit diesem Zeiger **Free**() auf. Sie möchten die `_tcsdup` ()-Lauf Zeit Bibliotheksfunktion verwenden, um die Zuordnung durchzuführen. Außerdem müssen Sie den dem aktuellen Zeiger zugeordneten Arbeitsspeicher freigeben, bevor Sie einen neuen Wert zuweisen. Beispiel:
...
> [!NOTE]
> Wenn Sie einen Wert zuweisen `m_pszProfileName` , muss er dynamisch auf dem Heap zugeordnet werden. Der `CWinApp` Dekonstruktor Ruft mit diesem Zeiger **Free**() auf. Sie möchten die `_tcsdup` ()-Lauf Zeit Bibliotheksfunktion verwenden, um die Zuordnung durchzuführen. Außerdem müssen Sie den dem aktuellen Zeiger zugeordneten Arbeitsspeicher freigeben, bevor Sie einen neuen Wert zuweisen. Beispiel:
...
Wenn Sie einen Wert zuweisen `m_pszRegistryKey` , muss er dynamisch auf dem Heap zugeordnet werden. Der `CWinApp` Dekonstruktor Ruft mit diesem Zeiger **Free**() auf. Sie möchten die `_tcsdup` ()-Lauf Zeit Bibliotheksfunktion verwenden, um die Zuordnung durchzuführen. Außerdem müssen Sie den dem aktuellen Zeiger zugeordneten Arbeitsspeicher freigeben, bevor Sie einen neuen Wert zuweisen. Beispiel:
...
Sie müssen der Klassen Meldungs Zuordnung eine `ON_COMMAND( ID_CONTEXT_HELP, OnContextHelp )` -Anweisung hinzufügen `CWinApp` und außerdem einen Zugriffstasten-Tabelleneintrag hinzufügen (normalerweise UMSCHALT + F1), um diese Element Funktion zu aktivieren.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_FILE_NEW, OnFileNew )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Funktion aktiviert ist, wird die Ausführung des Datei neuen Befehls behandelt.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_FILE_OPEN, OnFileOpen )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Funktion aktiviert ist, wird die Ausführung des Datei Öffnungs Befehls behandelt.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_FILE_PRINT_SETUP, OnFilePrintSetup )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Funktion aktiviert ist, wird die Ausführung des File Print-Befehls behandelt.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_HELP, OnHelp )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Option aktiviert ist, wird Sie von Framework aufgerufen, wenn der Benutzer die F1-Taste drückt.

Die Standard Implementierung dieser nachrichtenhandlerfunktion bestimmt den Hilfe Kontext, der dem aktuellen Fenster, Dialogfeld oder Menü Element entspricht, und ruft dann WINHELP.EXE auf. Wenn derzeit kein Kontext verfügbar ist, verwendet die Funktion den Standardkontext.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_HELP_FINDER, OnHelpFinder )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Option aktiviert ist, ruft das Framework diese nachrichtenhandlerfunktion auf, wenn der Benutzer der Anwendung den Befehl help Finder zum Aufrufen `WinHelp` mit dem Standard **HELP_FINDER** Thema auswählt.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_HELP_INDEX, OnHelpIndex )` `CWinApp` , um diese Member-Funktion zu aktivieren. Wenn diese Option aktiviert ist, ruft das Framework diese nachrichtenhandlerfunktion auf, wenn der Benutzer der Anwendung den Befehl "Hilfe Index" auswählt, der `WinHelp` mit dem Standard **HELP_INDEX** Thema aufgerufen werden soll.
...
Sie müssen der Klassen Meldungs Zuordnung eine-Anweisung hinzufügen `ON_COMMAND( ID_HELP_USING, OnHelpUsing )` `CWinApp` , um diese Member-Funktion zu aktivieren. Das Framework ruft diese nachrichtenhandlerfunktion auf, wenn der Benutzer der Anwendung den Befehl Hilfe using zum Aufrufen der `WinHelp` Anwendung mit dem Standard **HELP_HELPONHELP** Thema auswählt.
...
4. Schließlich `OnIdle` schließt die Verarbeitung aller Leerlauf Aufgaben ab und gibt 0 zurück. Dadurch wird der Nachrichten Schleife mitgeteilt, dass nicht mehr aufgerufen `OnIdle` wird, bis die nächste Nachricht von der Nachrichten Warteschlange empfangen wird. zu diesem Zeitpunkt wird der Leerlauf Zyklen neu gestartet, wobei das Argument auf 0 festgelegt ist.

Führen Sie während der Ausführung von keine langen Aufgaben `OnIdle` aus, da Ihre Anwendung Benutzereingaben erst verarbeiten kann, wenn `OnIdle` zurückgibt

> [!NOTE]
> Die Standard Implementierung von `OnIdle` aktualisiert Befehls Benutzeroberflächen Objekte, wie z. b. Menü Elemente und Symbolleisten Schaltflächen, und führt eine interne Bereinigung der Datenstruktur durch. Wenn Sie `OnIdle` überschreiben, müssen Sie daher `CWinApp::OnIdle` mit dem in der `lCount` überschriebenen Version aufzurufen. Zuerst wird die gesamte Basisklassen-Leerlauf Verarbeitung aufgerufen (d. h., bis die Basisklasse `OnIdle` 0 zurückgibt). Wenn Sie vor Abschluss der Verarbeitung der Basisklasse Arbeiten ausführen müssen, überprüfen Sie die Basisklassen Implementierung, um die richtige *lCount* -Anweisung auszuwählen, in der Sie Ihre Arbeit erledigen möchten.

Wenn Sie nicht `OnIdle` immer dann aufgerufen werden möchten, wenn eine Nachricht aus der Nachrichten Warteschlange abgerufen wird, können Sie [cwinthreadisidlemessage](../../mfc/reference/cwinthread-class.md#isidlemessage)überschreiben. Wenn eine Anwendung einen sehr kurzen Timer festgelegt hat, oder wenn das System die WM_SYSTIMER Nachricht sendet, `OnIdle` wird wiederholt aufgerufen, und die Leistung wird beeinträchtigt.

### <a name="example"></a>Beispiel

In den folgenden zwei Beispielen wird gezeigt, wie verwendet wird `OnIdle` . Im ersten Beispiel werden zwei Leerlauf Aufgaben verarbeitet, wobei das *lCount* -Argument verwendet wird, um die Aufgaben zu priorisieren. Die erste Aufgabe ist eine hohe Priorität, und Sie sollten Sie nach Möglichkeit verwenden. Die zweite Aufgabe ist weniger wichtig und sollte nur ausgeführt werden, wenn eine lange Pause in der Benutzereingabe vorliegt. Beachten Sie den-Befehl für die Basisklassen Version von `OnIdle` . Im zweiten Beispiel wird eine Gruppe von Leerlauf Tasks mit unterschiedlichen Prioritäten verwaltet.
...
1. Nachdem Sie in erstellt `InitInstance` wurde, `CCommandLineInfo` wird das-Objekt an das-Objekt geleitet `ParseCommandLine` .
...
Beachten Sie, dass Sie bei Bedarf direkt aufgerufen werden können `ParseCommandLine` .
...
Überschreiben Sie diese Funktion, um Fenster Meldungen zu filtern, bevor diese an die Windows-Funktionen [translatemess](/windows/win32/api/winuser/nf-winuser-translatemessage) und [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) gesendet werden. die Standard Implementierung führt die Zugriffstasten Übersetzung aus, sodass Sie die `CWinApp::PreTranslateMessage` Member-Funktion in der überschriebenen Version aufrufen müssen.
...
## <a name="cwinappprocessshellcommand"></a><a name="processshellcommand"></a> CWinApp::P rocess shellcommand
...
Wenn Sie ein neues MFC-Projekt mithilfe des Anwendungs-Assistenten starten, wird vom Anwendungs-Assistenten eine lokale Instanz von erstellt `CCommandLineInfo` , und anschließend wird `ProcessShellCommand` in der Member-Funktion und " [paramesecommandline](#parsecommandline) " aufgerufen `InitInstance` . Eine Befehlszeile folgt der unten beschriebenen Route:
...
3. `ParseParam` füllt das- `CCommandLineInfo` Objekt, das dann an weitergegeben wird `ProcessShellCommand` .

4. `ProcessShellCommand` behandelt die Befehlszeilenargumente und-Flags.

Die Datenmember des `CCommandLineInfo` Objekts, die durch [ccommandlineinfo:: m_nShellCommand](../../mfc/reference/ccommandlineinfo-class.md#m_nshellcommand)bezeichnet werden, sind vom folgenden Enumerationstyp, der in der- `CCommandLineInfo` Klasse definiert ist.
...
## <a name="cwinappprocesswndprocexception"></a><a name="processwndprocexception"></a> CWinApp::P rocess wndprocexception
...
Führt alle Registrierungs Tasks aus, die nicht von behandelt werden `RegisterShellFileTypes` .
...
Dadurch kann der Benutzer eine Datendatei öffnen, die von Ihrer Anwendung erstellt wurde, indem Sie im Datei-Manager darauf doppelklicken. Wird aufgerufen, `RegisterShellFileTypes` nachdem Sie [AddDocTemplate](#adddoctemplate) für jede Dokument Vorlage in der Anwendung aufgerufen haben. Außerdem wird die Funktion [EnableShellOpen](#enableshellopen) Member aufgerufen, wenn aufgerufen wird `RegisterShellFileTypes` .
...
Wenn in der Registrierungsdatenbank eine angegebene Dateinamenerweiterung bereits einem anderen Dateityp zugeordnet ist, wird keine neue Zuordnung erstellt. Das `CDocTemplate` Format der zum Registrieren dieser Informationen erforderlichen Zeichen folgen finden Sie in der-Klasse.
...
*bregisterwiederherstellungsrückruf*\
...
*"ertifizierer"*\
in Die eindeutige Zeichenfolge, die diese Instanz des Neustart-Managers identifiziert. Der Bezeichner für den Neustart-Manager ist für jede Instanz einer Anwendung eindeutig.
...
*lpvparam*\
in Der Eingabeparameter für die Wiederherstellungs Rückruffunktion. Weitere Informationen finden Sie unter [CWinApp:: applicationwiederherstellungsrückruf](#applicationrecoverycallback).
...
Wenn Ihre Anwendung die MFC-Standard Implementierung für das automatische Speichern von Dateien verwendet, sollten Sie die einfache Version von verwenden `RegisterWithRestartManager` . Verwenden Sie die komplexe Version von `RegisterWithRestartManager` , wenn Sie das Verhalten für die automatische Speicherung Ihrer Anwendung anpassen möchten.
...
Ein- **`int`** Wert, der von zurückgegeben wird `WinMain` .

### <a name="remarks"></a>Bemerkungen

`Run` Ruft Windows-Meldungen ab und sendet Sie, bis die Anwendung eine WM_QUIT Nachricht empfängt. Wenn die Nachrichten Warteschlange der Anwendung zurzeit keine Nachrichten enthält, `Run` ruft [OnIdle](#onidle) auf, um die Leerlaufzeit Verarbeitung auszuführen. Eingehende Nachrichten werden an die [pretranslatemess Age](#pretranslatemessage) -Member-Funktion für die spezielle Verarbeitung und dann an die Windows-Funktion `TranslateMessage` für die Standardtastatur Übersetzung gesendet `DispatchMessage` . Schließlich wird die Windows-Funktion aufgerufen.
...
## <a name="cwinappsethelpmode"></a><a name="sethelpmode"></a> CWinApp:: Setup Mode
...
Um den Hilfstyp Ihrer Anwendung auf HTMLHelp festzulegen, können Sie [enablehtmlhelp](#enablehtmlhelp)aufzurufen. Nachdem Sie aufgerufen `EnableHTMLHelp` haben, muss Ihre Anwendung HTMLHelp als Hilfe Anwendung verwenden. Wenn Sie die Verwendung von WinHelp ändern möchten, können Sie den Befehl `SetHelpMode` und *ehelptype* auf festlegen `afxWinHelp` .

## <a name="cwinappsetregistrykey"></a><a name="setregistrykey"></a> CWinApp:: abtregistrykey
...
## <a name="cwinappsupportsautosaveatinterval"></a><a name="supportsautosaveatinterval"></a> CWinApp:: supportsauessaveatinterval
...
## <a name="cwinappsupportsautosaveatrestart"></a><a name="supportsautosaveatrestart"></a> CWinApp:: supportsaudesaveatrestart
...
## <a name="cwinappwriteprofilebinary"></a><a name="writeprofilebinary"></a> CWinApp:: Beschreib teprofilebinary
...
In diesem Beispiel `CWinApp* pApp = AfxGetApp();` wird die CWinApp-Klasse verwendet, um zu veranschaulichen, dass `WriteProfileBinary` und `GetProfileBinary` von jeder Funktion in einer MFC-Anwendung verwendet werden können.

[!code-cpp[NVC_MFCWindowing#54](../../mfc/reference/codesnippet/cpp/cwinapp-class_29.cpp)]

Ein weiteres Beispiel finden Sie im Beispiel für [CWinApp:: getprofilebinary](#getprofilebinary).

## <a name="cwinappwriteprofileint"></a><a name="writeprofileint"></a> CWinApp:: Beschreib teprofileint
...
In diesem Beispiel wird `CWinApp* pApp = AfxGetApp();` die CWinApp-Klasse verwendet, um zu veranschaulichen, wie `WriteProfileString` , `WriteProfileInt` , `GetProfileString` und `GetProfileInt` von jeder Funktion in einer MFC-Anwendung verwendet werden können.

[!code-cpp[NVC_MFCWindowing#43](../../mfc/reference/codesnippet/cpp/cwinapp-class_9.cpp)]

Ein weiteres Beispiel finden Sie im Beispiel für [CWinApp:: GetProfileInt](#getprofileint).

## <a name="cwinappwriteprofilestring"></a><a name="writeprofilestring"></a> CWinApp:: Schreib Profil Zeichenfolge
...
## <a name="cwinappsetappid"></a><a name="setappid"></a> CWinApp:: "abtappid"
