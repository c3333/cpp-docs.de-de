...
Wenn Sie die von einem Kombinations Feld gesendeten Windows-Benachrichtigungs Meldungen an das übergeordnete Element (in der Regel eine von `CDialog`abgeleitete Klasse) verarbeiten möchten, fügen Sie der übergeordneten Klasse für jede Nachricht einen Meldungs Zuordnungs Eintrag und eine nachrichtenhandlermember-Funktion hinzu.

Jeder Nachrichten Zuordnungs Eintrag hat die folgende Form:

**Bei\_** _Benachrichtigung_ **(** _ID_, _mitgliedungsfxn_ **)**

Dabei gibt `id` die ID des untergeordneten Fensters des Kombinations Feld-Steuer Elements an, das die Benachrichtigung sendet, und `memberFxn` der Name der übergeordneten Element Funktion ist, die Sie zum Verarbeiten der Benachrichtigung geschrieben haben.
...
Wenn Sie ein `CComboBox` Objekt in einem Dialogfeld (über eine Dialogfeld Ressource) erstellen, wird das `CComboBox` Objekt automatisch zerstört, wenn der Benutzer das Dialogfeld schließt.

Wenn Sie ein `CComboBox` Objekt innerhalb eines anderen Fenster Objekts einbetten, müssen Sie es nicht zerstören. Wenn Sie das `CComboBox`-Objekt auf dem Stapel erstellen, wird es automatisch zerstört. Wenn Sie das `CComboBox`-Objekt auf dem Heap mithilfe der **New** -Funktion erstellen, müssen Sie **Delete** für das-Objekt erstellen, um es zu zerstören, wenn das Windows-Kombinations Feld zerstört wird.

**Hinweis** Wenn Sie WM_KEYDOWN und WM_CHAR Nachrichten verarbeiten möchten, müssen Sie die Steuerelemente des Kombinations Felds bearbeiten und auflisten, Klassen von `CEdit` und `CListBox`ableiten und den abgeleiteten Klassen Handler für diese Nachrichten hinzufügen. Weitere Informationen finden Sie unter [CWnd:: SubclassWindow](../../mfc/reference/cwnd-class.md#subclasswindow).
...
Wird von Framework aufgerufen, wenn der Benutzer ein Element aus einem Besitzer `CComboBox` Objekt löscht oder das Kombinations Feld zerstört.
...
Die Anzahl von Elementen. Die zurückgegebene Anzahl ist ein Wert größer als der Indexwert des letzten Elements (der Index ist NULL basiert). Dies ist CB_ERR, wenn ein Fehler auftritt.
...
Rufen Sie die `GetDroppedControlRect` Member-Funktion auf, um die Bildschirm Koordinaten des sichtbaren (Dropdown-) Listen Felds eines Dropdown-Kombinations Felds abzurufen.
...
Ruft die `GetDroppedState` Member-Funktion auf, um zu bestimmen, ob das Listenfeld eines Dropdown-Kombinations Felds sichtbar (Dropdown) ist.
...
Ruft die `GetExtendedUI` Member-Funktion auf, um zu bestimmen, ob ein Kombinations Feld über die Standardbenutzer Oberfläche oder die erweiterte Benutzeroberfläche verfügt.
...
Rufen Sie die `GetItemHeight` Member-Funktion auf, um die Höhe der Listenelemente in einem Kombinations Feld abzurufen.
...
Die Verwendung des CBS_OWNERDRAWFIXED Formats in einem mit der [SubclassDlgItem](../../mfc/reference/cwnd-class.md#subclassdlgitem) -Member-Funktion von `CWnd` erstellten Kombinations Feld "Besitzer zeichnen" umfasst weitere Überlegungen zur Programmierung. Weitere Informationen finden Sie in der [technischen Notiz 14](../../mfc/tn014-custom-controls.md).
...
Beachten Sie, dass die Funktionen `SelectString` und `FindString` Member beide eine Zeichenfolge suchen, aber die `SelectString` Member-Funktion wählt auch die Zeichenfolge aus.
...
Mit der `SetExtendedUI` Member-Funktion können Sie entweder die Standardbenutzer Oberfläche oder die erweiterte Benutzeroberfläche für ein Kombinations Feld auswählen, das die [CBS_DROPDOWN](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) oder [CBS_DROPDOWNLIST](../../mfc/reference/styles-used-by-mfc.md#combo-box-styles) Stil hat.
...
Legt den 32-Bit-Wert, der dem angegebenen Element in einem Kombinations Feld zugeordnet ist, als den angegebenen Zeiger fest (**void** <strong>\*</strong>).
