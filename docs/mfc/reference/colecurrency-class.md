...
|[COleCurrency::P arsecurrency](#parsecurrency)|Liest einen Currency-Wert aus einer Zeichenfolge und legt den Wert `COleCurrency`fest.|
...
|[Operator +,-](#operator_plus_minus)|Fügt `COleCurrency` Werten ein Vorzeichen hinzu, subtrahiert und ändert Sie.|
|[Operator + =,-=](#operator_plus_minus_eq)|Fügt einen `COleCurrency` Wert aus diesem `COleCurrency` Objekt hinzu und subtrahiert diesen.|
|[Operator */](#operator_star)|Skaliert einen `COleCurrency` Wert durch einen ganzzahligen Wert.|
|[Operator * =,/=](#operator_star_div_eq)|Skaliert diesen `COleCurrency` Wert um einen ganzzahligen Wert.|
|[Operator < <](#operator_stream)|Gibt einen `COleCurrency` Wert an `CArchive` oder `CDumpContext`aus.|
|[Operator > >](#operator_stream)|Gibt ein `COleCurrency` Objekt von `CArchive`ein.|
...
`COleCurrency` implementiert auch einige grundlegende arithmetische Operationen für diesen festpunkttyp. Die unterstützten Vorgänge wurden ausgewählt, um die Rundungsfehler zu steuern, die während der festgelegten Berechnung auftreten.
...
- COleCurrency (`varSrc`) erstellt ein `COleCurrency` Objekt. Versucht, eine [Variant](/windows/win32/api/oaidl/ns-oaidl-variant) -Struktur oder ein `COleVariant` Objekt in einen Currency-Wert (VT_CY) zu konvertieren. Wenn diese Konvertierung erfolgreich ist, wird der konvertierte Wert in das neue `COleCurrency` Objekt kopiert. Wenn dies nicht der Fall ist, wird der Wert des `COleCurrency` Objekts auf NULL (0) und seinen Status auf ungültig festgelegt.
...
Eine `CString`, die den formatierten Währungswert enthält.
...
Mit dieser Member-Funktion können Sie den Status (Gültigkeit) eines angegebenen `COleCurrency` Objekts abrufen.
...
Der Rückgabewert wird durch den `CurrencyStatus` enumerierten Typ definiert, der in der `COleCurrency`-Klasse definiert ist.
...
Eine kurze Beschreibung dieser Statuswerte finden Sie in der folgenden Liste:
  ...
  - `COleCurrency::null` gibt an, dass dieses `COleCurrency` Objekt NULL ist, d. h., dass kein Wert für dieses Objekt angegeben wurde. (Dies ist "Null", wenn kein Wert vorhanden ist, und nicht C++ NULL.)
...
Der Typ dieses Datenmembers ist der enumerierte Typ `CurrencyStatus`, der in der `COleCurrency` Klasse definiert ist.
...
> [!CAUTION]
>  Dieser Datenmember ist für erweiterte Programmier Situationen vorgesehen. Sie sollten die Inline Member-Funktionen [GetStatus](#getstatus) und [SetStatus](#setstatus)verwenden. Weitere Vorsichtsmaßnahmen zum expliziten Festlegen dieses Datenmembers finden Sie unter `SetStatus`.
...
- **Operator = (** `curSrc` **)** Der Wert und der Status des Operanden, ein vorhandenes `COleCurrency` Objekt werden in dieses `COleCurrency` Objekt kopiert.

- **Operator = (** *varSrc* **)** Wenn die Konvertierung des `VARIANT` Werts (oder des [COleVariant](../../mfc/reference/colevariant-class.md) -Objekts) in eine Währung (`VT_CY`) erfolgreich ist, wird der konvertierte Wert in dieses `COleCurrency` Objekt kopiert, und sein Status ist auf "gültig" festgelegt. Wenn die Konvertierung nicht erfolgreich ist, wird der Wert des `COleCurrency` Objekts auf 0 und dessen Status auf ungültig festgelegt.
...
Mit diesen Operatoren können Sie zwei `COleCurrency` Werte zu und voneinander subtrahieren und das Vorzeichen eines `COleCurrency` Werts ändern.
...
Ermöglicht das Hinzufügen und Subtrahieren eines `COleCurrency` Werts zu diesem `COleCurrency` Objekt.
...
Wenn der `COleCurrency` Operanden NULL ist, ist der Status des resultierenden `COleCurrency` Werts Null.
...
Mit dieser Member-Funktion werden die Einheiten und der Bruchteile dieses `COleCurrency` Objekts festgelegt.
