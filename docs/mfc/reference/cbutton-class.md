...
Außerdem unterstützt die von `CButton` abgeleitete [CBitmapButton](../../mfc/reference/cbitmapbutton-class.md) -Klasse die Erstellung von Schaltflächen-Steuerelementen, die mit Bitmapbildern anstelle von Text bezeichnet werden. Eine `CBitmapButton` kann separate Bitmaps für den Status "aufwärts", "herunter", "fokussiert" und "deaktiviert" einer Schaltfläche haben.

Sie können ein Schaltflächen-Steuerelement entweder aus einer Dialogfeld Vorlage oder direkt im Code erstellen. In beiden Fällen wird zuerst der Konstruktor aufgerufen, `CButton`, um das `CButton` Objekt zu erstellen; Rufen Sie dann die `Create` Member-Funktion auf, um das Windows-Schaltflächen Steuerelement zu erstellen, und fügen Sie es an das `CButton`

Die Konstruktion kann ein einstufiger Prozess in einer Klasse sein, die von `CButton`abgeleitet ist. Schreiben Sie einen Konstruktor für die abgeleitete Klasse, und nennen Sie `Create` aus dem Konstruktor.
...
Wenn Sie ein `CButton` Objekt aus einer Dialogfeld Ressource erstellen, wird das `CButton` Objekt automatisch zerstört, wenn der Benutzer das Dialogfeld schließt.

Wenn Sie ein `CButton` Objekt innerhalb eines Fensters erstellen, müssen Sie es möglicherweise zerstören. Wenn Sie das `CButton`-Objekt auf dem Heap mithilfe der **New** -Funktion erstellen, müssen Sie **Delete** für das-Objekt verwenden, um es zu zerstören, wenn der Benutzer das Windows-Schaltflächen-Steuerelement schließt. Wenn Sie das `CButton`-Objekt auf dem Stapel erstellen oder es in das übergeordnete Dialog Objekt eingebettet ist, wird es automatisch zerstört.
...
*Rect*<br/>
Gibt die Größe und Position des Schaltflächen Steuer Elements an. Dabei kann es sich entweder um ein `CRect` Objekt oder um eine `RECT` Struktur handeln.

*pparser*<br/>
Gibt das übergeordnete Fenster des Schaltflächen Steuer Elements an, in der Regel eine `CDialog`. Er darf nicht NULL sein.
...
Mit dieser Methode können Sie den Textrand des `CButton` Objekts abrufen.
...
Im Unterschied zu [CBitmapButton](../../mfc/reference/cbitmapbutton-class.md), das vier Bitmaps pro Schaltfläche verwendet, `SetBitmap` nur eine Bitmap pro Schaltfläche verwendet. Wenn die Schaltfläche gedrückt wird, wird die Bitmap nach unten und nach rechts verschoben.
...
Im Unterschied zu [CBitmapButton](../../mfc/reference/cbitmapbutton-class.md), das vier Bitmaps pro Schaltfläche verwendet, `SetCursor` nur einen Cursor pro Schaltfläche verwendet. Wenn die Schaltfläche gedrückt wird, wird der Cursor nach unten und nach rechts verschoben.
...
Legt den Zustand des aktuellen Schaltflächen-Steuer Elements auf `elevation required`fest. Dies ist erforderlich, damit das-Steuerelement ein Symbol mit erhöhten Rechten anzeigen kann.
...
Im Unterschied zu [CBitmapButton](../../mfc/reference/cbitmapbutton-class.md), das vier Bitmaps pro Schaltfläche verwendet, `SetIcon` nur ein Symbol pro Schaltfläche verwendet. Wenn die Schaltfläche gedrückt wird, wird das Symbol nach unten und nach rechts verschoben.
...
Diese Methode initialisiert den `mask` Member einer [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) -Struktur mit dem BCSIF_GLYPH-Flag und dem `himlGlyph`-Element mit dem *chglyph* -Parameter und sendet diese Struktur dann in der [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) Nachricht, die in der Windows SDK beschrieben wird.
...
Diese Methode initialisiert den `mask` Member einer [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) -Struktur mit dem BCSIF_IMAGE-Flag und dem `himlGlyph`-Element mit dem *psplitimagelist* -Parameter und sendet diese Struktur dann in der [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) Nachricht, die in der Windows SDK beschrieben wird.
...
Im folgenden Codebeispiel wird die-Variable definiert, `m_splitButton`, die verwendet wird, um Programm gesteuert auf das Steuerelement für die geteilte Schaltfläche zuzugreifen.
...
Diese Methode initialisiert den `mask` Member einer [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) -Struktur mit dem BCSIF_SIZE-Flag und dem `size`-Element mit dem *Psize* -Parameter und sendet diese Struktur dann in der [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) Nachricht, die in der Windows SDK beschrieben wird.
...
Die Stile der unterteilten Schaltflächen geben die Ausrichtung, das Seitenverhältnis und das grafische Format an, mit dem Windows ein Symbol für eine unterteilte Schaltfläche zeichnet Weitere Informationen finden Sie in der `uSplitStyle`-Member der [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) -Struktur.

Diese Methode initialisiert den `mask` Member einer [BUTTON_SPLITINFO](/windows/win32/api/commctrl/ns-commctrl-button_splitinfo) -Struktur mit dem BCSIF_STYLE-Flag und dem `uSplitStyle`-Element mit dem *usplitstyle* -Parameter und sendet diese Struktur dann in der [BCM_GETSPLITINFO](/windows/win32/Controls/bcm-getsplitinfo) Nachricht, die in der Windows SDK beschrieben wird.
