...
1. Wenn während der Ausführung des geschützten Abschnitts oder in einer Routine, die der geschützte Abschnitt aufruft, eine Ausnahme auftritt, wird der **__except** -Ausdruck ausgewertet. Es gibt drei mögliche Werte:
...
Die Zeiger Typen `PEXCEPTION_RECORD` und `PCONTEXT` werden in der Includedatei \<"Winnt. h>" `_EXCEPTION_RECORD` definiert `_CONTEXT` und in der Includedatei \<"excpt. h" definiert>

Sie können innerhalb `GetExceptionCode` des Ausnahme Handlers verwenden. Sie können jedoch nur innerhalb `GetExceptionInformation` des Ausnahme Filter Ausdrucks verwenden. Die Informationen, auf die Sie verweist, werden im Allgemeinen auf dem Stapel angezeigt und sind nicht mehr verfügbar, wenn die Steuerung an den Ausnahmehandler übertragen wird.
...
\<excpt. h> definiert einige alternative Namen für diese intrinsie:
