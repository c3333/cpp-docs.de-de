...
description: Das C++ Build Insights SDK TRACING_SESSION_MSVC_EVENT_FLAGS Konstantenreferenz.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
Die `TRACING_SESSION_MSVC_EVENT_FLAGS` Konstanten werden verwendet, um zu beschreiben, welche MSVC-Ereignisse während einer Ablaufverfolgung gesammelt werden sollen. Verwenden Sie sie, [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) um das `MsvcEventFlags` Feld der TRACING_SESSION_OPTIONS Struktur zu initialisieren.
...
| `TRACING_SESSION_MSVC_EVENT_FLAGS_BACKEND_FUNCTIONS` | [FORCE_INLINEE](../event-table.md#force-inlinee)<br/>[Funktion](../event-table.md#function) |
