...
ms.locfileid: "79544483"
---
# <a name="how-to-migrate-to-clr"></a>Gewusst wie: Migrieren zu /clr

In diesem Thema werden Probleme erläutert, die beim Kompilieren von System eigenem Code mit **/CLR** auftreten. Weitere Informationen finden Sie unter [/CLR (Common Language Runtime-Kompilierung)](../build/reference/clr-common-language-runtime-compilation.md) . **/CLR** ermöglicht System C++ eigenen Code das Aufrufen und Aufrufen von .NET-Assemblys zusätzlich zu C++ anderem nativem Code. Weitere Informationen zu den Vorteilen der Kompilierung mit **/CLR**finden Sie unter [gemischte (Native und verwaltete)](../dotnet/mixed-native-and-managed-assemblies.md) Assemblys und [Native und .NET-Interoperabilität](../dotnet/native-and-dotnet-interoperability.md) .
...
Wenn Sie von einer Version vor Visual Studio 2003 auf Visual Studio 2010 aktualisieren, werden möglicherweise Compilerfehler im Zusammenhang mit der C++ verbesserten Standardkonformität in Visual Studio 2003 angezeigt.
...
Obwohl Visual Studio C-Dateien kompiliert, ist es notwendig, Sie für eine C++ **/CLR** -Kompilierung in zu konvertieren. Der tatsächliche Dateiname muss nicht geändert werden. Sie können **/tp** verwenden (siehe [/TC,/TP,/TC,/TP (Quell Dateityp angeben)](../build/reference/tc-tp-tc-tp-specify-source-file-type.md).) Obwohl C++ Quell Code Dateien für **/CLR**erforderlich sind, ist es nicht notwendig, den Code für die Verwendung objektorientierter Paradigmen umzugestalten.
...
Bezeichner, die in C-Code verwendet werden, bei C++ dem es sich um Schlüsselwörter in handelt (z. b. **Virtual**, **New**, **Delete**, **bool**, **true**, **false**usw.), müssen umbenannt werden. Dies kann im Allgemeinen durch einfache Such- und Ersetzungsvorgänge erfolgen.
...
`__controlfp` wird im Common Language Runtime nicht unterstützt (Weitere Informationen finden Sie unter [_control87, _controlfp](../c-runtime-library/reference/control87-controlfp-control87-2.md) \__control87_2). Die CLR respektiert auch keine [Ausrichtung](../cpp/align-cpp.md).
