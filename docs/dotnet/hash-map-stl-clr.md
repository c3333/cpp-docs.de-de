...
`GKey` ist identisch `Key` mit, es sei denn, der letztere ist ein Ref-Typ. in diesem Fall ist es `Key^`

`GMapped` ist identisch `Mapped` mit, es sei denn, der letztere ist ein Ref-Typ. in diesem Fall ist es `Mapped^`
...
Ein hash_map unterstützt Bidirektionale Iteratoren. Dies bedeutet, dass Sie bei einem Iterator, der ein Element in der gesteuerten Sequenz festlegt, zu angrenzenden Elementen wechseln können. Ein spezieller Haupt Knoten entspricht dem Iterator, der von [hash_map:: End (STL/CLR)](#end)zurückgegeben wurde `()` . Sie können diesen Iterator verringern, um das letzte Element in der kontrollierten Sequenz zu erreichen, falls vorhanden. Sie können einen hash_map Iterator Inkrement erhöhen, um den Head-Knoten zu erreichen, und dann wird gleich verglichen `end()` . Sie können jedoch nicht den von zurückgegebenen Iterator dereferenzieren `end()` .
...
Die Member-Funktion ruft effektiv [hash_map:: Erase (STL/CLR)](#erase) `(` [hash_map:: begin (STL/CLR)](#begin) `(),` [hash_map:: End (STL/CLR](#end)) auf `())` . Sie verwenden ihn, um sicherzustellen, dass die gesteuerte Sequenz leer ist.
...
Die Memberfunktion gibt „true“ für eine leere gesteuerte Sequenz zurück. Dies entspricht [hash_map:: Size (STL/CLR)](#size) `() == 0` . Sie verwenden es, um zu testen, ob das hash_map leer ist.
...
Die Member-Funktion gibt ein paar von Iteratoren zurück `cliext::pair<iterator, iterator>(lower_bound(key), upper_bound(key))` . Sie verwenden Sie, um den Bereich der Elemente zu bestimmen, die sich derzeit in der kontrollierten Sequenz befinden, die einem angegebenen Schlüssel entsprechen.
...
Wenn mindestens ein Element in der kontrollierten Sequenz eine äquivalente Reihenfolge mit *Key*aufweist, gibt die Member-Funktion einen Iterator zurück, der eines dieser Elemente festlegt. Andernfalls wird [hash_map:: End (STL/CLR)](#end)zurückgegeben `()` . Sie verwenden Sie, um ein Element zu suchen, das sich derzeit in der kontrollierten Sequenz befindet, die einem angegebenen Schlüssel entspricht.
...
Die Member-Funktion gibt `(float)` [hash_map:: Size (STL/CLR)](#size) `() /` [hash_map:: bucket_count (STL/CLR)](#bucket_count)zurück `()` . Sie verwenden Sie, um die durchschnittliche Bucket-Größe zu bestimmen.
