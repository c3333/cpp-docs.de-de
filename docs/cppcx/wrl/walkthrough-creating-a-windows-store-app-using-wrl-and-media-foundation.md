...
- Die Attribute "`namespace`" und "`runtimeclass`" sowie der `NTDDI_WIN8`[Version](/windows/win32/Midl/version) -Attribut Wert sind wichtige Teile der Mittell-Definition für eine Media Foundation Komponente, die WRL verwendet.
...
1. Verwenden Sie den folgenden Code, um die Inhalte `pch.h`zu ersetzen:
...
1. Verwenden Sie den folgenden Code, um die Inhalte `GrayscaleTransform.cpp`zu ersetzen:

   [!code-cpp[wrl-media-capture#4](../codesnippet/CPP/walkthrough-creating-a-windows-store-app-using-wrl-and-media-foundation_4.cpp)]

1. Fügen Sie dem Projekt eine neue Modul Definitionsdatei hinzu, benennen Sie Sie `GrayscaleTransform.def`, und fügen Sie dann den folgenden Code hinzu:
...
1. Verwenden Sie den folgenden Code, um die Inhalte `dllmain.cpp`zu ersetzen:
...
1. Legen Sie im Dialogfeld Eigenschaften **Seiten** des Projekts die folgenden **Linker** -Eigenschaften fest.
   ...
   1. Geben Sie unter **Eingabe**für die **Modul Definitionsdatei**`GrayScaleTransform.def`an.
...
1. Fügen Sie ein neues  **C# Projekt für eine leere app (Universal Windows)** zur `MediaCapture` Projekt Mappe hinzu. Geben Sie dem Projekt einen Namen, z. b. *mediacapture*.
...
1. Wählen Sie in `Package.appxmanifest`auf der Registerkarte **Funktionen** die Option **Mikrofon** und **Webcam**aus. Beide Funktionen sind erforderlich, um Fotos von der Webcam zu erfassen.

1. Fügen Sie den folgenden Code in `MainPage.xaml`dem Stamm [Raster](/uwp/api/Windows.UI.Xaml.Controls.Grid) Element hinzu:

   [!code-xml[wrl-media-capture#7](../codesnippet/Xaml/walkthrough-creating-a-windows-store-app-using-wrl-and-media-foundation_7.xaml)]

1. Verwenden Sie den folgenden Code, um die Inhalte `MainPage.xaml.cs`zu ersetzen:
