...
description: Referenzhandbuch für CRT-Funktionen, die in universellen Windows-Plattform-Apps nicht unterstützt werden.
ms.date: 04/16/2020
...
Viele C-Laufzeitfunktionen (CRT) sind nicht verfügbar, wenn Sie UWP-Apps (Universelle Windows-Plattform) erstellen. Manchmal sind Problemumgehungen verfügbar, z. B. können Sie Windows-Runtime- oder Win32-APIs verwenden. In anderen Fällen wurden CRT-Funktionen gesperrt, da die entsprechenden Funktionen oder die unterstützenden APIs nicht auf UWP-Apps anwendbar sind. Informationen zur Suche nach einer alternativen Methode, die für die Windows-Runtime unterstützt wird, finden Sie unter [Alternativen zu Windows-APIs in UWP-Apps](/uwp/win32-and-com/alternatives-to-windows-apis-uwp).

In der folgenden Tabelle sind die CRT-Funktionen aufgeführt, die beim Erstellen von UWP-Apps nicht verfügbar sind. Er zeigt alle Problemumgehungen an, die angewendet werden.
...
|_loaddll _getdllprocaddr _unloaddll|Dies waren veraltete Funktionen in früheren CRT-Versionen. Außerdem kann ein Benutzer DLLs nur mit denen im gleichen Anwendungspaket laden.|Verwenden Sie die Win32-APIs `LoadPackagedLibrary`, `GetProcAddress`und `FreeLibrary` zum Laden und Verwenden verpackter DLL-Dateien.|
...
Die folgenden Funktionen sind in der CRT für UWP-Apps verfügbar. Verwenden Sie sie jedoch nur, wenn Sie die entsprechenden Win32- oder Windows-Runtime-APIs nicht verwenden können, z. B. wenn Sie große Codebasen portieren:

|||
|-|-|
|Einzelbyte-Zeichenfolgenfunktionen, z. B. `strcat`, `strcpy`, `strlwr`usw.|Machen Sie Ihre UWP-Apps ausschließlich Unicode, da alle Win32-APIs und Windows-Runtime-APIs, die verfügbar gemacht werden, nur Unicode-Zeichensätze verwenden.  Single-Byte-Funktionen wurden für die Portierung großer Codebasen übrig gelassen, sollten aber ansonsten vermieden werden. Die entsprechenden Wide-Char-Funktionen sollten stattdessen verwendet werden, wenn möglich.|
|Stream-E/A- und Datei-E/A-Funktionen auf niedriger Ebene, z. B. `fopen`, `open`usw.|Diese Funktionen sind synchron, was für UWP-Apps nicht empfohlen wird. Verwenden Sie in Ihren Apps für die universelle Windows-Plattform asynchrone APIs, um Dateien zu öffnen, aus Dateien zu lesen und in Dateien zu schreiben, damit das Sperren des UI-Threads verhindert wird. Beispiele für derartige APIs finden Sie in der `Windows::Storage::FileIO` -Klasse.|

## <a name="windows-8x-store-apps-and-windows-phone-8x-apps"></a>Windows 8.x Store-Apps und Windows Phone 8.x-Apps

Sowohl die zuvor genannten APIs als auch die folgenden APIs sind in Windows 8.x Store-Apps und Windows Phone 8.x-Apps nicht verfügbar.
