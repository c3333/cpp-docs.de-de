...
Alle Konstruktoren speichern ein `_Allocator` zuordnerobjekt und initialisieren die ungeordnete Multimenge.
...
Der vierte und fünfte Konstruktor geben eine Kopie der gleichzeitigen ungeordneten Multimenge an `_Uset` .

Der letzte Konstruktor gibt eine Verschiebung der gleichzeitigen ungeordneten Multimenge an `_Uset` .
...
Nachdem alle vorhandenen Elemente in einer gleichzeitigen ungeordneten Multimenge gelöscht wurden, `operator=` kopiert oder verschiebt den Inhalt von `_Uset` in die gleichzeitige ungeordnete Multimenge.
