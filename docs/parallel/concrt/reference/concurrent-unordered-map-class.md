...
|Name|Beschreibung|
|----------|-----------------|
|[KOM\[\]](#operator_at)|Ist überladen. Sucht ein Element mit dem angegebenen Schlüssel oder fügt es ein. Diese Methode ist nebenläufigkeitssicher.|
...
Der dritte Konstruktor gibt Werte an, die vom iteratorbereich [,) bereitgestellt werden `_Begin` `_End` .

Der vierte und fünfte Konstruktor geben eine Kopie der gleichzeitigen ungeordneten Zuordnung an `_Umap` .

Der letzte Konstruktor gibt eine Verschiebung der gleichzeitigen ungeordneten Zuordnung an `_Umap` .
...
**`true`**, wenn der parallele Container leer ist, **`false`** andernfalls.
...
Die erste Member-Funktion bestimmt, ob ein Element X in der Sequenz vorhanden ist, deren Schlüssel eine entsprechende Reihenfolge aufweist `value` . Wenn dies nicht der Fall ist, wird ein solches Element X erstellt und mit initialisiert `value` . Die-Funktion bestimmt dann den Iterator `where` , der X festlegt. Wenn eine Einfügung aufgetreten ist, gibt die Funktion zurück `std::pair(where, true)` . Andernfalls wird `std::pair(where, false)`zurückgegeben.
...
Die letzten zwei Element Funktionen Verhalten sich identisch mit den ersten beiden, außer dass `value` zum Erstellen des eingefügten Werts verwendet wird.
...
`operator[]`kann zum Einfügen von Elementen in eine Zuordnung mit verwendet werden `m` `m[key] = DataValue;` , wobei `DataValue` der Wert des `mapped_type` Elements mit einem Schlüsselwert von ist `key` .

Wenn `operator[]` zum Einfügen von Elementen verwendet wird, gibt der zurückgegebene Verweis nicht an, ob eine Einfügung ein bereits vorhandenes Element ändert oder ein neues erstellt. Mithilfe der `find` Element Funktionen und der [Insert](#insert) -Funktion kann festgestellt werden, ob ein Element mit einem angegebenen Schlüssel bereits vor einer Einfügung vorhanden ist.
...
Nach dem Löschen vorhandener Elemente ein gleichzeitiger Vektor `operator=` kopiert oder verschiebt den Inhalt von `_Umap` in den gleichzeitigen Vektor.
...
Die erste Member-Funktion entfernt das-Element der kontrollierten Sequenz, auf die von verwiesen wird `_Where` . Die zweite Member-Funktion entfernt die Elemente im Bereich [ `_Begin` , `_End` ).

Die dritte Member-Funktion entfernt die Elemente im durch `concurrent_unordered_map::equal_range` (kval) getrennten Bereich.
