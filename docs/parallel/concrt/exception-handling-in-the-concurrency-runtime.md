...
- [Leichte Aufgaben](#lwts)

- [Asynchrone Agents](#agents)

## <a name="tasks-and-continuations"></a><a name="tasks"></a>Aufgaben und Fortsetzungen
...
" [parallelcurrency:: task_canceled](../../parallel/concrt/reference/task-canceled-class.md) " ist ein wichtiger Lauf Zeit Ausnahmetyp, der sich auf bezieht `task` . Die Runtime löst `task_canceled` aus, wenn Sie `task::get` aufrufen und diese Aufgabe abgebrochen wird. (Im Gegensatz dazu `task::wait` gibt [task_status:: abgebrochen](reference/concurrency-namespace-enums.md#task_group_status) zurück und löst nicht aus.) Sie können diese Ausnahme von einer aufgabenbasierten Fortsetzung oder beim Ausführen von abfangen und behandeln `task::get` . Weitere Informationen zum Aufgaben Abbruch finden Sie unter [Abbruch in der ppl](cancellation-in-the-ppl.md).
...
## <a name="task-groups-and-parallel-algorithms"></a><a name="task_groups"></a>Aufgaben Gruppen und parallele Algorithmen
...
## <a name="exceptions-thrown-by-the-runtime"></a><a name="runtime"></a>Von der Laufzeit ausgelöste Ausnahmen
...
## <a name="multiple-exceptions"></a><a name="multiple"></a>Mehrere Ausnahmen
...
## <a name="cancellation"></a><a name="cancellation"></a>Streichung
...
## <a name="lightweight-tasks"></a><a name="lwts"></a>Leichte Aufgaben
...
## <a name="asynchronous-agents"></a><a name="agents"></a>Asynchrone Agents
...
Die- `run` Methode umgibt alle Aufgaben in einem- **`try`** - **`catch`** Block. Der- **`catch`** Block speichert die Ausnahme in einem Nachrichten Puffer. Die Anwendung überprüft, ob im Agent ein Fehler aufgetreten ist. Hierzu wird dieser Puffer nach Abschluss des Agents gelesen.
...
Da der- **`try`** - **`catch`** Block außerhalb der- **`while`** Schleife vorhanden ist, beendet der Agent die Verarbeitung, wenn er auf den ersten Fehler stößt. Wenn sich der **`try`** - **`catch`** Block in der **`while`** Schleife befunden hat, wird der Agent nach einem Fehler fortgesetzt.
