...
*T*<br/>
Die Klasse, die das Steuerelement implementiert und von abgeleitet wird `CStockPropImpl` .
...
*plizierung*<br/>
Ein Zeiger auf die LIBID der Typbibliothek, die die Definition von enthält `InterfaceName` .
...
Aus Gründen der Abwärtskompatibilität macht `CStockPropImpl` auch `get_Window` die-Methode und die-Methode verfügbar, `put_Window` die einfach `get_HWND` und `put_HWND` bzw. Die Standard Implementierung von `put_HWND` gibt E_FAIL zurück, da HWND eine schreibgeschützte Eigenschaft sein soll.
...
Die gleichen drei vordefinierten Eigenschaften erfordern, dass Ihr zugehöriger Datenmember vom Typ `CComPtr` oder einer anderen Klasse ist, die mithilfe des Zuweisungs Operators die korrekte Zählung der Schnittstellen Verweise bereitstellt.
